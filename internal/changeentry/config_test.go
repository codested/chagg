...
// ── ReleasePolicy defaults and merging ────────────────────────────────────────

func TestDefaultReleasePolicyHasAutoValues(t *testing.T) {
	p := defaultReleasePolicy()
	if p.VPrefix != "auto" {
		t.Fatalf("expected VPrefix=auto, got %q", p.VPrefix)
	}
	if p.AliasTags != "auto" {
		t.Fatalf("expected AliasTags=auto, got %q", p.AliasTags)
	}
}

func TestMergeReleasePolicyOverridesVPrefix(t *testing.T) {
	base := defaultReleasePolicy()
	result := mergeReleasePolicy(base, RawReleasePolicy{VPrefix: "always"})
	if result.VPrefix != "always" {
		t.Fatalf("expected VPrefix=always, got %q", result.VPrefix)
	}
	if result.AliasTags != "auto" {
		t.Fatalf("expected AliasTags unchanged=auto, got %q", result.AliasTags)
	}
}

func TestMergeReleasePolicyOverridesAliasTags(t *testing.T) {
	base := defaultReleasePolicy()
	result := mergeReleasePolicy(base, RawReleasePolicy{AliasTags: "never"})
	if result.VPrefix != "auto" {
		t.Fatalf("expected VPrefix unchanged=auto, got %q", result.VPrefix)
	}
	if result.AliasTags != "never" {
		t.Fatalf("expected AliasTags=never, got %q", result.AliasTags)
	}
}

func TestMergeReleasePolicyOverridesBoth(t *testing.T) {
	base := defaultReleasePolicy()
	result := mergeReleasePolicy(base, RawReleasePolicy{VPrefix: "never", AliasTags: "always"})
	if result.VPrefix != "never" {
		t.Fatalf("expected VPrefix=never, got %q", result.VPrefix)
	}
	if result.AliasTags != "always" {
		t.Fatalf("expected AliasTags=always, got %q", result.AliasTags)
	}
}

func TestMergeReleasePolicyEmptyRawLeavesBaseUnchanged(t *testing.T) {
	base := ReleasePolicy{VPrefix: "always", AliasTags: "never"}
	result := mergeReleasePolicy(base, RawReleasePolicy{})
	if result.VPrefix != "always" {
		t.Fatalf("expected VPrefix=always, got %q", result.VPrefix)
	}
	if result.AliasTags != "never" {
...
func TestMergeReleasePolicyNormalizesCase(t *testing.T) {
	base := defaultReleasePolicy()
	result := mergeReleasePolicy(base, RawReleasePolicy{VPrefix: "ALWAYS", AliasTags: "  Never  "})
	if result.VPrefix != "always" {
		t.Fatalf("expected lowercase always, got %q", result.VPrefix)
	}
	if result.AliasTags != "never" {
		t.Fatalf("expected lowercase never, got %q", result.AliasTags)
	}
}

func TestResolveModuleForChangesDirDefaultReleasePolicyIsAuto(t *testing.T) {
	tempDir := t.TempDir()
	repoDir := filepath.Join(tempDir, "repo")
	changesDir := filepath.Join(repoDir, ".changes")
	...
	if module.Release.VPrefix != "auto" {
		t.Fatalf("expected Release.VPrefix=auto, got %q", module.Release.VPrefix)
	}
	if module.Release.AliasTags != "auto" {
		t.Fatalf("expected Release.AliasTags=auto, got %q", module.Release.AliasTags)
	}
}

func TestResolveModuleForChangesDirAppliesReleasePolicyFromRepoConfig(t *testing.T) {
	tempDir := t.TempDir()
	repoDir := filepath.Join(tempDir, "repo")
	changesDir := filepath.Join(repoDir, ".changes")
	...
	if module.Release.VPrefix != "always" {
		t.Fatalf("expected Release.VPrefix=always, got %q", module.Release.VPrefix)
	}
	if module.Release.AliasTags != "never" {
		t.Fatalf("expected Release.AliasTags=never, got %q", module.Release.AliasTags)
	}
}
