...
func defaultReleasePolicy() ReleasePolicy {
	return ReleasePolicy{VPrefix: "auto", AliasTags: "auto"}
}

func mergeReleasePolicy(base ReleasePolicy, raw RawReleasePolicy) ReleasePolicy {
	result := base
	if v := strings.ToLower(strings.TrimSpace(raw.VPrefix)); v != "" {
		result.VPrefix = v
	}
	if v := strings.ToLower(strings.TrimSpace(raw.AliasTags)); v != "" {
		result.AliasTags = v
	}
	return result
}
...
func newResolvedLayer() resolvedLayer {
	...
	return resolvedLayer{types: cp, gitPolicy: defaultGitWritePolicy(), releasePolicy: defaultReleasePolicy()}
}
...
func (l *resolvedLayer) applyRelease(raw RawReleasePolicy) {
	l.releasePolicy = mergeReleasePolicy(l.releasePolicy, raw)
}
...
func ResolveModuleForChangesDir(repoRoot string, changesDir string) (ModuleConfig, error) {
	...
	if repoCfg != nil {
		...
		if rawMod != nil {
			if err := applyRawModuleOverrides(&layer, rawMod, &name, &tagPrefix, configName); err != nil {
				return ModuleConfig{}, err
			}
			...
			if err := validateInferredLayout(repoRoot, absChangesDir); err != nil {
				return ModuleConfig{}, err
			}
		}
	} else {
	...
	return buildModuleConfig(name, absChangesDir, tagPrefix, layer), nil
}
...
func ResolveModulesForChangesDirs(repoRoot string, changesDirs []string) (map[string]ModuleConfig, error) {
	...
	for _, changesDir := range changesDirs {
		...
		if repoCfg != nil {
			...
			if rawMod != nil {
				if err := applyRawModuleOverrides(&ml, rawMod, &name, &tagPrefix, configName); err != nil {
					return nil, err
				}
			} else if hasConfig {
				return nil, NewValidationError("config",
		...
		result[changesDir] = buildModuleConfig(name, absChangesDir, tagPrefix, ml)
	}
...
// applyRawModuleOverrides merges a raw module's config onto layer and updates
// name/tagPrefix when explicitly set.
func applyRawModuleOverrides(layer *resolvedLayer, rawMod *RawModule, name *string, tagPrefix *string, configName string) error {
	if err := layer.applyTypes(rawMod.Types); err != nil {
		return fmt.Errorf("%s module %q types: %w", configName, rawMod.Name, err)
	}
	layer.applyDefaults(rawMod.Defaults)
	layer.applyRelease(rawMod.Release)
...
// validateInferredLayout discovers all .changes dirs and validates that
// auto-inferred module names don't collide and root/nested layouts aren't mixed.
func validateInferredLayout(repoRoot string, absChangesDir string) error {
	discoveredDirs, err := gitutil.FindAllChangesDirs(repoRoot)
	if err != nil {
		return err
	}
	...
	if err := validateNoMixedRootAndNested(repoRoot, discoveredDirs); err != nil {
		return err
	}
	return validateInferredModuleNames(repoRoot, discoveredDirs)
}

// buildModuleConfig assembles a ModuleConfig from its resolved parts.
func buildModuleConfig(name, changesDir, tagPrefix string, layer resolvedLayer) ModuleConfig {
	return ModuleConfig{
		Name:       name,
...
func buildBaseLayer(repoRoot string) (resolvedLayer, *RawConfig, string, error) {
	...
	if userRaw != nil {
		...
		layer.applyRelease(userRaw.Release)
	...
	if repoCfg != nil {
		...
		layer.applyRelease(repoCfg.Release)
