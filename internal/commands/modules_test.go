...
func TestRenderModulesJSON(t *testing.T) {
	...
	if len(decoded.Modules) != 1 {
		t.Fatalf("expected 1 entry, got %d", len(decoded.Modules))
	}
	if decoded.Modules[0].Name != "api" {
		t.Fatalf("expected name 'api', got %q", decoded.Modules[0].Name)
	}
	if decoded.Modules[0].ChangesDir != "api/.changes" {
		t.Fatalf("expected changes_dir 'api/.changes', got %q", decoded.Modules[0].ChangesDir)
	}
	if decoded.Modules[0].TagPrefix != "api-" {
		t.Fatalf("expected tag_prefix 'api-', got %q", decoded.Modules[0].TagPrefix)
	}
}
...
func TestRenderModulesJSONEmpty(t *testing.T) {
	...
	if len(decoded.Modules) != 0 {
		t.Fatalf("expected empty array, got %d entries", len(decoded.Modules))
	}
}
