...
func loadEntries(repoRoot string, module changeentry.ModuleConfig, tags []semver.Tag) ([]EntryWithMeta, []InvalidEntry, error) {
	...
	// Parse all entries first, collecting valid paths for the batch git lookup.
	...
	for _, path := range paths {
		...
		parsed = append(parsed, parsedEntry{path: path, entry: entry})
	}

	// Batch git lookup: single subprocess call for all valid files.
	validPaths := make([]string, len(parsed))
	for i, p := range parsed {
		validPaths[i] = p.path
	}
	batchMeta := gitutil.FileAddedMetaMany(repoRoot, validPaths)

	entries := make([]EntryWithMeta, 0, len(parsed))
	for _, p := range parsed {
		var addedAt time.Time
		var addedCommitHash, originalFilename string
		var hasGit bool

		if meta, ok := batchMeta[p.path]; ok {
			addedAt = meta.AddedAt
			addedCommitHash = meta.AddedCommitHash
			originalFilename = meta.OriginalFilename
			hasGit = true
		} else {
			// Fallback for files not found in batch (renamed files need --follow).
			addedAt, addedCommitHash, originalFilename, hasGit = gitutil.FileAddedMeta(repoRoot, p.path)
		}

		version := resolveVersion(p.entry, addedAt, hasGit, tags)
		slog.Debug("resolved entry version", "file", filepath.Base(p.path), "version", version)
